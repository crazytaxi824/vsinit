...
import { Table, useTableView, useRefreshComponent } from './mytable';
    ...
    <Grid container sx={{ height: 60 }}>
      ...
      >
        {state.selection.length > 0 && (
          <div style={{ marginLeft: '20px' }}>
            {state.selection.length} row(s) selected
          </div>
        )}
      </Grid>
      ...
      <Grid item xs={8} container justifyContent="center" alignItems="center">
        <Pagination
          ...
          count={Math.ceil(
            props.table.resp.data.rowCount / props.table.req.pageSize
          )}
...
export function MyDataTable(): JSX.Element {
  console.log('mytable');
